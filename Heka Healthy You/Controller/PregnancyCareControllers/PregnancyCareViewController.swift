...
// BUTTON SHOULD BE ENABLE WHEN DETAILS ARE PREFILLED---DONE
// OTP LOGIC ISSUE --- DONE
// ALL IMAGES 4X ACCORDING TO NEW DESIGN
// CONSTRAINT ISSUE IN WHY CHOSE US OF SAFETY TRAINING----DONE
// ACTUAL CODED STARS FOR RATING AND CLICK ON ADD STAR
// SPLASH SCREEN---- DONE

import UIKit

class PregnancyCareViewController: MenuViewController {
    ...
    var safetyImages: [String] = ["ImgPregnancyCare"]
    var serviceImages: [String] = ["Prenatal", "Post", "Continuum"]
    var serviceNames: [String] = ["Prenatal Care", "Post Pregnanacy", "Continuum Care"]
...
extension PregnancyCareViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == safetyCollectionView {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "PregnancySafetyCVC", for: indexPath) as! PregnancySafetyCVC
            cell.imgSafetyTraining.image =  UIImage(named: safetyImages[indexPath.row])
            return cell
        } else {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "PregnancyServiceCVC", for: indexPath) as! PregnancyServiceCVC
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView == serviceCollectionView {
            if indexPath.item == 0 {
                let vc = storyboard?.instantiateViewController(withIdentifier: "PrenatalCareViewController") as! PrenatalCareViewController
                self.navigationController?.pushViewController(vc, animated: true)
                
            } else if indexPath.item == 1 {
                let vc = storyboard?.instantiateViewController(withIdentifier: "PostPregnancyViewController") as! PostPregnancyViewController
                self.navigationController?.pushViewController(vc, animated: true)
            } else {
                let vc = storyboard?.instantiateViewController(withIdentifier: "ContinuumCareViewController") as! ContinuumCareViewController
                self.navigationController?.pushViewController(vc, animated: true)
            }
        }
