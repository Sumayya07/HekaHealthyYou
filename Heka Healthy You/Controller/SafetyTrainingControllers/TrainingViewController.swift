...
class TrainingViewController: MenuViewController {
    ...
    override func viewDidLoad() {
        ...
        btnBook.layer.cornerRadius = 6
        btnBook.layer.shadowColor = UIColor.gray.cgColor
        btnBook.layer.shadowOffset = CGSize(width: 0, height: 1)
        btnBook.layer.shadowOpacity = 1
        btnBook.layer.shadowRadius = 3
...
extension TrainingViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == safetyCollectionView {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "SafetyTrainingCVC", for: indexPath) as! SafetyTrainingCVC
            cell.imgSafetyTraining.image =  UIImage(named: safetyImages[indexPath.row])
            return cell
        } else {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "ServiceInformationCVC", for: indexPath) as! ServiceInformationCVC
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView == serviceCollectionView {
            if indexPath.item == 0 {
                let vc = storyboard?.instantiateViewController(withIdentifier: "FirstAidViewController") as! FirstAidViewController
                self.navigationController?.pushViewController(vc, animated: true)
                
            } else if indexPath.item == 1 {
                let vc = storyboard?.instantiateViewController(withIdentifier: "RoadSafetyViewController") as! RoadSafetyViewController
                self.navigationController?.pushViewController(vc, animated: true)
            } else if indexPath.item == 2 {
                let vc = storyboard?.instantiateViewController(withIdentifier: "BLSTrainingViewController") as! BLSTrainingViewController
                self.navigationController?.pushViewController(vc, animated: true)
            } else {
                let vc = storyboard?.instantiateViewController(withIdentifier: "ElectricalSafetyViewController") as! ElectricalSafetyViewController
                self.navigationController?.pushViewController(vc, animated: true)
            }
        }
